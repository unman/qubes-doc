...
For each AppVM, there is a pair of ``qubes-gui`` (running in AppVM) and ``qubes-guid`` (running in the AppVM’s interface qube, dom0 by default) processes connected over vchan. The main responsibilities of ``qubes-gui`` are:
